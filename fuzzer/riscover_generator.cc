...
const std::vector<xed_reg_enum_t> kFullPool = {
    ...
    XED_REG_R12, XED_REG_R13, XED_REG_R14, XED_REG_R15,XED_REG_RIP
};
...
const std::vector<xed_iclass_enum_t> kSupportedIclass = {
    ...
    XED_ICLASS_CMP, XED_ICLASS_TEST, XED_ICLASS_CALL
  };
...
bool RiscoverGenerator::GenerateInstruction(Rng& rng, uint8_t* buf, size_t& len) {
...
bool RiscoverGenerator::GenerateRegReg(Rng& rng, xed_iclass_enum_t iclass, 
                                       unsigned int width, uint8_t* buf, size_t& len) {
